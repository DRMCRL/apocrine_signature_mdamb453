...
```{r}
gr <- dht_consensus %>% 
  filter(!!!syms(targets), mapped) %>% 
  filter(
    vapply(gene_name, function(x) "MYC" %in% x[1], logical(1))
  ) %>% 
  resize(width = 1.5*width(.), fix = 'center')
# gr <- subset(all_gr$gene, gene_name == "MYC")
myc_plot <- plotHFGC(
  gr = gr,
  hic = hic %>%
    subsetByOverlaps(
    ...
    subset(as.integer(bin_size) < 4e4),
  hicsize = 3,
  # max = 1.5e6,
  genes = tm %>%
  ...
  rotation.title = 90, fontsize = 14, drawAxis = FALSE
)
myc_plot$Detected@dp@pars$rotation.title <- 0
myc_plot$Detected@dp@pars$fontsize.group <- 14
myc_plot$Detected@dp@pars$cex.group <- 0.8
myc_plot$`Not-Detected`@dp@pars$rotation.title <- 0
myc_plot$`Not-Detected`@dp@pars$fontsize.group <- 14
...
myc_plot$AR@dp@pars$drawAxis <- FALSE
myc_plot$FOXA1@dp@pars$drawAxis <- FALSE
myc_plot$GATA3@dp@pars$drawAxis <- FALSE
myc_plot$TFAP2B@dp@pars$drawAxis <- FALSE
ax <- myc_plot[1:2]
hl <- HighlightTrack(myc_plot[3:10], range = gr)
hl@dp@pars$fill <- rgb(1, 1, 1, 0)
hl@dp@pars$col <- "blue"
pdf(here::here("output", "MYC.pdf"), width = 7, height = 7)
Gviz::plotTracks(
  c(ax, hl),
  myc_plot[1:10], 
