...
bwfl <- list(
  AR = here::here("data", "bigwig", "AR") %>% 
    list.files(pattern = "merged", full.names = TRUE) %>% 
    BigWigFileList() %>% 
    setNames(
      path(.) %>% 
        basename() %>% 
...
plotHFGC(
  ...
  hic = hic %>%
    subset(
      overlapsAny(., gr) &
        overlapsAny(., subset(all_gr$gene, gene_name == "MYC")) &
        as.integer(bin_size) < 4e4
    ),
  hicsize = 3,
  genes = tm %>% 
    mutate(detected = ifelse(gene %in% detected, "Detected", "Not-Detected")) %>% 
    split(.$detected),
  genecol = c(Detected = "forestgreen", "Not-Detected" = "grey"),
  coverage = bwfl, ylim = lapply(y_lim, divide_by, e2 = 2),
