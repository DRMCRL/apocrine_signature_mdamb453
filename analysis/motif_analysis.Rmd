...
meme_db_targets <- meme_db_detected %>% 
  ...
  to_list()
## We should generate these as a complete set & using H3K27ac status...
seq_by_targets <- grl_by_targets %>% 
  # endoapply(subset, H3K27ac) %>% 
  get_sequence(hg19)
...
```{r ame-heatmap, fig.cap = "Estimated percentage of true positive matches to all binding motifs associated with each ChIP target. Only values with an enrichment adjusted p-value < 0.05 and with True Positives in > 15% of sequences are shown, such that missing values are able to be confidently assumed as not enriched for the motif. With the possible exception of GATA3 binding in the presence of AR and FOXA1 only, all provide supporting evidence for direct binding of each target to it's motif"}
ame_by_targets %>% 
  bind_rows(.id = "targets") %>% 
  mutate(
...
```{r ame-by-detected, eval=FALSE}
ame_by_detected <- seq_by_targets %>% 
  runAme(database = to_list(meme_db_detected))
id <- ame_by_detected %>%
  lapply(
    dplyr::filter, 
    tp_percent > 50, 
    !motif_alt_id %in% targets, 
    !str_detect(motif_alt_id, "(TFAP2[A-Z]|GATA[0-9]|FOX[A-Z][0-9])")
    ) %>% 
  bind_rows(.id = "targets") %>% 
...
tomtom_all4_by_h3k27ac[streme_to_keep_all4_by_h3k27ac] %>% 
  ...
  dplyr::select(
    ...
    IC, `Matches Found` = nsites, `P-value` = pval, `E-value` = eval,
  ...
  fmt_number("Matches Found", decimals = 0, use_seps = TRUE) %>% 
  ...
  tab_spanner(
    label = "Statistics",
    columns = c("IC", "Matches Found", "P-value", "E-value")
  ) %>% 
  text_transform(
...
<!-- ### Novel Motif Co-Ocurrence -->

```{r, eval = FALSE, echo = FALSE}
views_all4_h3k27ac <- seq_all4_by_h3k27ac$h3k27ac %>% 
  as("DNAStringSet") %>% 
  as("Views") %>% 
  setNames(
    names(seq_all4_by_h3k27ac$h3k27ac)
  )
best_match_streme_all4 <- streme_all4_by_h3k27ac %>% 
  dplyr::slice(
    which(streme_to_keep_all4_by_h3k27ac)
  ) %>% 
  pull("motif") %>% 
  lapply(getBestMatch, subject = views_all4_h3k27ac) %>% 
  setNames(
    streme_all4_by_h3k27ac$altname[streme_to_keep_all4_by_h3k27ac]
  ) %>% 
  bind_rows(.id = "name") %>% 
  arrange(view) 
best_match_streme_all4 %>% 
  mutate(value = 1) %>% 
  pivot_wider(names_from = motif, values_fill = 0, id_cols = "range") %>% 
  dplyr::select(starts_with("STREME")) %>% 
  as.matrix() %>% 
  cor(method = "spearman") %>% 
  corrplot::corrplot(
    order = "hclust", diag = FALSE, type = "lower", 
...
### Novel Motif Centrality {.tabset}

Using the same approach as before, i.e. looking the lens of each ChIP target's binding peak, any proximity effect with each ChIP target was assessed.

#### AR

```{r ar-streme-centrality}
streme_ar_summits <- streme_all4_by_h3k27ac %>% 
  dplyr::slice(which(streme_to_keep_all4_by_h3k27ac)) %>% 
  to_list %>%
  lapply(
    getBestMatch, 
    subject = views_ar_summits,
  ...
  arrange(view) %>% 
  dplyr::select(
    range, all_of(targets), H3K27ac, altname, 
    pos = start_in_view, width, strand, seq
  ) %>% 
  left_join(
    tomtom_all4_by_h3k27ac[streme_to_keep_all4_by_h3k27ac] %>% 
      bind_rows() %>% 
      dplyr::select(altname, best_match = best_match_altname),
    by = "altname"
  ) 
```

```{r plot-centrality-ar-streme, fig.height = 6, fig.cap = "Position of the best match for each novel, H3K27ac-enriched motif using sequences centred at the summits of AR binding sites. Solid lines denote sites where H3K27ac signal was detected, whilst dashed lines indicate no H3K27ac activity. Numbers at the bottom of each panel indicate the total number of sites with a match within the AR sites overlapping H3K27ac signal."}
streme_ar_summits  %>%
  mutate(
    label = glue("{altname}\n({best_match})"),
    H3K27ac = ifelse(H3K27ac, "H3K27ac", "No H3K27ac")
  ) %>% 
  ggplot(aes(pos, stat(density))) +
  geom_density(aes(linetype = H3K27ac)) +
  geom_text(
    aes(x = seq_width/2, y = 5e-4, label = lab),
    data = . %>%
      dplyr::filter(H3K27ac == "H3K27ac") %>% 
      group_by(label) %>%
      summarise(
        n = dplyr::n(), .groups = "drop"
      ) %>%
      mutate(
        lab = glue(
            "n = {comma(n)} ({percent(n/sum(ar_summits$H3K27ac), 0.1)})"
          )
      ),
    colour = "black",
    inherit.aes = FALSE
  ...
  labs(
    x = "Best Match Position In Sequence",
    y = "Density",
    linetype = "Overlaps H3K27ac"
  ) +
  theme(
    legend.position = c(2/3, 1/6)
  )
```

Across all novel motifs no discernible difference was noticed in the positional distribution between sites showing H3K27ac signal and those not.
However clear positional effects were evident for some of these motifs in relation to the AR binding peak.

#### FOXA1

```{r foxa1-streme-centrality}
streme_foxa1_summits <- streme_all4_by_h3k27ac %>% 
  dplyr::slice(which(streme_to_keep_all4_by_h3k27ac)) %>% 
  to_list %>%
  lapply(
    getBestMatch, 
...
```{r plot-centrality-foxa1-streme, fig.height = 6, fig.cap = "Position of the best match for each novel, H3K27ac-enriched motif using sequences centred at the summits of FOXA1 binding sites. Solid lines denote sites where H3K27ac signal was detected, whilst dashed lines indicate no H3K27ac activity. Numbers at the bottom of each panel indicate the total number of sites with a match within the FOXA1 sites overlapping H3K27ac signal."}
streme_foxa1_summits  %>%
  mutate(
    label = glue("{altname}\n({best_match})"),
    H3K27ac = ifelse(H3K27ac, "H3K27ac", "No H3K27ac")
  ) %>% 
  ...
  geom_text(
    ...
    data = . %>%
      ...
      mutate(
        lab = glue(
            "n = {comma(n)} ({percent(n/sum(foxa1_summits$H3K27ac), 0.1)})"
          )
      ),
    colour = "black",
    inherit.aes = FALSE
...
Across all novel motifs no discernible difference was noticed in the positional distribution between sites showing H3K27ac signal and those not.
However clear positional effects were evident for all these motifs in relation to the FOXA1 binding peak.

#### GATA3

```{r gata3-streme-centrality}
streme_gata3_summits <- streme_all4_by_h3k27ac %>% 
  dplyr::slice(which(streme_to_keep_all4_by_h3k27ac)) %>% 
  to_list %>%
  lapply(
    getBestMatch, 
...
```{r plot-centrality-gata3-streme, fig.height = 6, fig.cap = "Position of the best match for each novel, H3K27ac-enriched motif using sequences centred at the summits of GATA3 binding sites. Solid lines denote sites where H3K27ac signal was detected, whilst dashed lines indicate no H3K27ac activity. Numbers at the bottom of each panel indicate the total number of sites with a match within the GATA3 sites overlapping H3K27ac signal."}
streme_gata3_summits  %>%
  mutate(
    label = glue("{altname}\n({best_match})"),
    H3K27ac = ifelse(H3K27ac, "H3K27ac", "No H3K27ac")
  ) %>% 
  ...
  geom_text(
    ...
    data = . %>%
      ...
      mutate(
        lab = glue(
            "n = {comma(n)} ({percent(n/sum(gata3_summits$H3K27ac), 0.1)})"
          )
      ),
    colour = "black",
    inherit.aes = FALSE
...
Across all novel motifs no discernible difference was noticed in the positional distribution between sites showing H3K27ac signal and those not.
However clear positional effects were evident for some these motifs in relation to the GATA3 binding peak.

#### TFAP2B

```{r tfap2b-streme-centrality}
streme_tfap2b_summits <- streme_all4_by_h3k27ac %>% 
  dplyr::slice(which(streme_to_keep_all4_by_h3k27ac)) %>% 
  to_list %>%
  lapply(
    getBestMatch, 
...
```{r plot-centrality-tfap2b-streme, fig.height = 6, fig.cap = "Position of the best match for each novel, H3K27ac-enriched motif using sequences centred at the summits of TFAP2B binding sites. Solid lines denote sites where H3K27ac signal was detected, whilst dashed lines indicate no H3K27ac activity. Numbers at the bottom of each panel indicate the total number of sites with a match within the TFAP2B sites overlapping H3K27ac signal."}
streme_tfap2b_summits  %>%
  mutate(
    label = glue("{altname}\n({best_match})"),
    H3K27ac = ifelse(H3K27ac, "H3K27ac", "No H3K27ac")
  ) %>% 
  ...
  geom_text(
    ...
    data = . %>%
      ...
      mutate(
        lab = glue(
            "n = {comma(n)} ({percent(n/sum(tfap2b_summits$H3K27ac), 0.1)})"
          )
      ),
    colour = "black",
    inherit.aes = FALSE
...
Across all novel motifs no discernible difference was noticed in the positional distribution between sites showing H3K27ac signal and those not.
However clear positional effects were evident for all these motifs in relation to the TFAP2B binding peak.
